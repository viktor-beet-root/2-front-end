...
var Course = /*#__PURE__*/function () {
  ...
  _createClass(Course, [{
    ...
    key: "changeSudent",
    value: function changeSudent(idStudent, newStudent) {
      var indexSudent = this.getStudentIndexById(idStudent);
      newStudent.id = idStudent;
      return this.studentsList.splice(indexSudent, 1, newStudent);
    } // 2. Метот повертає всі скіли студентів

  }, {
    key: "getAllStudSkills",
    value: function getAllStudSkills() {
      var res = [];
    ...
    key: "getUniqueStudSkillsList",
    value: // 3. Метод повертає унікальний список скілів студунтів
    function getUniqueStudSkillsList() {
      var allSkillslist = this.getAllStudSkills();
      var uniqueSkillsList = [];
      uniqueSkillsList.push(allSkillslist[0]);

      for (var i = 0; i < allSkillslist.length; i++) {
    ...
    key: "sortStudenstBySkills",
    value: function sortStudenstBySkills(option) {
      var _this = this;

      var amountSkillsStud = [];
      var tempArr = [];
      var res = [];
      ...
      var sortArr = amountSkillsStud.filter(function (item, i, arr) {
      ...
      var _loop = function _loop(_i) {
        if (option === true) {
          tempArr.unshift(_this.studentsList.filter(function (item) {
            if (item.skillList.length === sortArr[_i]) {
              return item;
            }
          }));
        }
        ...
        if (option === false) {
          tempArr.push(_this.studentsList.filter(function (item) {
            if (item.skillList.length === sortArr[_i]) {
              return item;
            }
          }));
      ...
      return res.concat.apply(res, tempArr);
    } //5. Метод шукає найтсаршого або наймолодшого студента в групі

  }, {
    key: "findMostOldStud",
    value: function findMostOldStud(param) {
      var ageList = this.studentsList.map(function (item) {
        return item.age;
      });
      var paramAge = ageList[0];
      ...
      return this.studentsList.filter(function (item) {
        return item.age === paramAge;
      });
    } // 6. Метод виводить студентів в заданому діапазоні віку

  }, {
    key: "findRangAgeStud",
    value: function findRangAgeStud(from, to) {
      var res = [];
      var rangeArr = [];
      ...
      for (var _i2 = 0; _i2 < rangeArr.length; _i2++) {
        for (var j = 0; j < this.studentsList.length; j++) {
          if (this.studentsList[j].age.indexOf(rangeArr[_i2]) !== -1) {
...
/* harmony import */ var _course__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! ./course */ "./src/js/course.js");
...
(function () {
  ...
  console.log(createLi()); // 2. Вивести інформацію про курс і сптсок студентів

  var front = new _course__WEBPACK_IMPORTED_MODULE_13__["default"]({
  ...
  document.querySelector('.container').append(cElem({
    tag: 'h3',
    class: 'title__course',
    content: ['Назава курсу: ' + front.courseName]
  }));
  document.querySelector('.container').append(cElem({
    tag: 'h4',
    content: ['Початок курсу: ' + front.start]
  }));
  document.querySelector('.container').append(cElem({
    tag: 'p',
    content: ["Список студентів у групі"]
  }));
  document.querySelector('.container').append(cElem({
    tag: 'ol',
    class: 'student-list'
  ...
  var studList = document.querySelector('.student-list');

  for (var i = 0; i < front.studentsList.length; i++) {
  ...
  var spans = document.querySelectorAll('.student-list li span');
  console.log(spans);
