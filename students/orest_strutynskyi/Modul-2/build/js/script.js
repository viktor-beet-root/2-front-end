...
var _loop = function _loop(i) {
  if (window.screen.width < 576) {
    elements[i].addEventListener('click', function () {
      var item = elements[i].querySelector('.sub-menu');
...
var swiper = new swiper__WEBPACK_IMPORTED_MODULE_10__["default"](".mySwiper", {
  speed: 700,
  modules: [swiper__WEBPACK_IMPORTED_MODULE_10__.Navigation, swiper__WEBPACK_IMPORTED_MODULE_10__.Pagination, swiper__WEBPACK_IMPORTED_MODULE_10__.Scrollbar],
  spaceBetween: 30,
  effect: "fade",
  navigation: {
    nextEl: ".swiper-button-next",
...
function addSmile(selector, selector2) {
  ...
  var smile = document.querySelectorAll(selector2);
  var eventInput = new Event('input', {
    bubbles: true,
    cancelable: true
  });
  smile.forEach(function (element) {
...
function smileToImg(textArea) {
  if (textArea.includes('[::')) {
    var indexStart = textArea.indexOf('[::');
    var images = {
      keys: [],
      srcs: []
    };
    var imageList = document.querySelectorAll('.massage__img');
    ...
    do {
      var indexEnd = textArea.indexOf('::]') + 3;
      var smileName = textArea.slice(indexStart + 3, indexEnd - 3);
      var endString = textArea.slice(indexEnd);
      textArea = textArea.slice(0, indexStart);
      var idSrc = images.keys.indexOf(smileName);

      if (~idSrc) {
        var str = images.srcs[idSrc];
        textArea = textArea + " <img src=\"".concat(str, "\" alt=\"\"> ");
      } else {
...
function validateTextarea() {
  ...
  var resultTextarea = document.querySelector('.textarea-result');
  var limit = 500;
  resultTextarea.textContent = "0 / ".concat(limit);
  textArea.addEventListener('input', function () {
    var textLength = textArea.value.length;
    resultTextarea.textContent = "".concat(textLength, " '/' ").concat(limit);
  });
}

validateTextarea(); //TODO FORM

var formComment = document.querySelector('.form-comment');
var errorClassName = 'error';
var errorMessage = 'This field is required';
var requiredElementList = formComment.querySelectorAll('.required:not(div)');
var post = document.querySelector('.button__post');
requiredElementList.forEach(function (e) {
...
function requiredCheck(element, userName, userEmail, userSite, textarea) {
  element.addEventListener('blur', function () {
    ...
    var postButton = this.closest('.form-comment').querySelector('.button__post');
    ...
    if (value === '') {
      ...
      if (this.classList.contains(userName)) {
        var validator = validateUserName(value);
      ...
      if (this.classList.contains(userEmail)) {
        var validatorEmail = validateUserEmail(value);
      ...
      if (this.classList.contains(userSite)) {
        var validatorSite = validateUserWebsite(value);
      ...
      if (this.classList.contains(textarea)) {
        var validatorTexrArea = validateTextArea(value);
    ...
    var requiredName = document.querySelector('.' + userName).getAttribute('data-required');
    var requiredEmail = document.querySelector('.' + userEmail).getAttribute('data-required');
    var requiredSite = document.querySelector('.' + userSite).getAttribute('data-required');
    var requiredTextarea = document.querySelector('.' + textarea).getAttribute('data-required');
...
function validateUserEmail(userEmail) {
  var validateEmail = /^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-z]+)$/;
...
scroll.addEventListener('click', function () {
  var scrollUp = document.querySelector(".page");
...
var buttonPost = document.querySelector('.form-comment');
buttonPost.addEventListener('submit', function (e) {
  ...
  if (checkbox.checked) {
    ...
    var userSite = document.querySelector('.userSite').value;
    var newUser = {
      userName: userName,
      userEmail: userEmail,
      userSite: userSite
    ...
    localStorage.setItem('user', JSON.stringify(userList)); //console.log(JSON.parse(localStorage.getItem('user')));
  }
});
var userList1 = JSON.parse(localStorage.getItem('user'));

if (userList1 !== null) {
  var lastUser = userList1[userList1.length - 1];
...
function renderPosts() {
  var maxId = -1;
  var maxReplyId = -1;
  var localStorageItem = JSON.parse(localStorage.getItem('userMessages'));

  if (localStorageItem) {
    localStorageItem.forEach(function (e) {
      messageToHtml(e.userName, e.userSite, e.textArea, e.getTime, e.id, e.counterLikes, e.liked);
      maxId = maxId > e.id ? maxId : e.id;
      var rPosts = e.repliedPosts;
      var place = document.querySelector('.comment__wrapper-hero');

      if (rPosts[0]) {
        rPosts.forEach(function (item) {
  ...
  var htmlStorage = JSON.parse(localStorage.getItem('inHtml'));

  if (htmlStorage) {
    htmlStorage.forEach(function (e) {
      var htmlComent = document.querySelector("button[data-id=".concat(e.id, "]"));

      if (e.isRemoved) {
        htmlComent.closest('.comment').remove();
      } else if (e.liked) {
        var htmlLikes = document.querySelector(".comment__like-wrapper>button[data-id=".concat(e.id, "]"));
        var likeCounter = htmlLikes.nextElementSibling;
      ...
      if (e.repliedPosts[0]) {
        e.repliedPosts.forEach(function (item) {
          var place = document.querySelector("[data-id=\"".concat(e.id, "\"]")).closest('.comment__wrapper-hero');
          replyToHtml(place, item.userName, item.replyName, item.userSite, item.textArea, item.getTime, e.id);
          maxReplyId = maxReplyId > item.id ? maxReplyId : item.id;
        });
        replyId = maxReplyId;
...
renderPosts(); //TODO	 Add comment

function messageToHtml(userName, userSite, textArea, time, id, counterLikes, liked) {
  var lastComent = document.querySelector('.messages-list');
  var addComment = document.createElement("li");
  addComment.className = 'messages-list__item comment';
  var html = "\n\t<div class=\"comment__wrapper comment-margin\">\n\t\t<div class=\"comment__wrapper-hero\">\n\t\t\t<div class=\"comment__header\">\n\t\t\t\t<div class=\"comment__ava ava\">\n\t\t\t\t\t<div class=\"ava__img1\">\n\t\t\t\t\t\t<img class=\"ava__img1-size\" src=\"./images/face-1.png\" alt=\"\">\n\t\t\t\t\t</div>\n\t\t\t\t\t<a class=\"ava__web\" href=\"\">".concat(userSite, "</a>\n\t\t\t\t</div>\n\t\t\t\t<div class=\"comment__group\">\n\t\t\t\t\t<h4 class=\"comment__title\">").concat(userName, "</h4>\n\t\t\t\t\t<div class=\"comment__publish-date\">").concat(timeCheck(time), "</div>\n\t\t\t\t</div>\n\t\t\t\t<div class=\"comment__actions\">\n\t\t\t\t\t<button class=\"edit\" data-id=\"").concat(id, "\">\n\t\t\t\t\t\t<img src=\"./images/edit-icon.svg\">\n\t\t\t\t\t</button>\n\t\t\t\t\t<button class=\"remove\" data-id=\"").concat(id, "\">\n\t\t\t\t\t\t<img src=\"./images/remove-icon.svg\">\n\t\t\t\t\t</button>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t\t<div class=\"comment__content\">\n\t\t\t\t<div class=\"comment__text\">").concat(textArea, " </div>\n\t\t\t\t<div class=\"comment__like-wrapper\">");

  if (!liked) {
    html = html + "<button class=\"like\" data-id=\"".concat(id, "\">\n\t\t<img src=\"./images/white-dark-h.svg\"></img>");
  } else {
    html = html + "<button class=\"like\" data-id=\"".concat(id, "\" disabled>\n\t\t<img src=\"./images/dark-dark-heart.svg\"></img>");
  }

  html = html + "\n\t\t\t\t\t</button>\n\t\t\t\t\t<p class=\"like-number\">".concat(counterLikes, "</p>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t\t<div class=\"comment__footer\">\n\t\t\t\t<button class=\"reply-img\" data-id=\"").concat(id, "\">Reply</button>\n\t\t\t\t<div class=\"top\">\n\t\t\t\t\t<button class=\"top-p\" data-id=\"").concat(id, "\">\n\t\t\t\t\t\t<img src=\"./images/like-icon.png\" >\n\t\t\t\t\t</button>\n\t\t\t\t\t<button class=\"top-m\" data-id=\"").concat(id, "\">\n\t\t\t\t\t\t<img src=\"./images/disLike.png\">\n\t\t\t\t\t</button>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t</div>\n");
...
function timeCheck(time) {
  ...
  if (timeMs < 60) {
    return "".concat(Math.floor(timeMs), " Minutes ago");
  }

  var timeH = timeMs / 60;

  if (timeH < 24) {
    return "".concat(Math.floor(timeH), " Hours ago");
  }

  var timeD = timeH / 24;

  if (timeD < 7) {
    return "".concat(Math.floor(timeD), " Days ago");
  }

  var timeW = timeD / 7;

  if (timeW < 4) {
    return "".concat(Math.floor(timeW), " Weeks ago");
  }

  var timeM = timeW / 4;

  if (timeM < 12) {
    return "".concat(Math.floor(timeM), " Months ago");
  }

  var timeY = timeM / 12;
  return "".concat(Math.floor(timeY), " Years ago");
}

var addLikeList = document.querySelectorAll('.like');

function addCommentPost() {
  var userName = document.querySelector('.userName').value;
  var userEmail = document.querySelector('.userEmail').value;
  var userSite = document.querySelector('.userSite').value;
  var textArea = smileToImg(document.querySelector('.textarea').value);
  var getTime = setTimeForm();
  var setLocalStorage = {
    liked: false,
    userName: userName,
    userEmail: userEmail,
  ...
  var localStorageMessages = [];
  var localStorageItem = localStorage.getItem('userMessages');
...
post.addEventListener('click', addCommentPost); //TODO: remove comments

function removeLike() {
  var htmlStorageList = [];
  var removeMessageList = JSON.parse(localStorage.getItem('userMessages'));
  var buttonRemoveList = document.querySelectorAll('.remove');
  buttonRemoveList.forEach(function (e) {
    e.addEventListener('click', function () {
      var id = Number(e.getAttribute('data-id'));

      if (isNaN(id)) {
        var formalId = e.getAttribute('data-id');
        ...
        var storageObject = htmlStorageList.find(function (item) {
          return item.id === formalId;
        });
        ...
        var found = removeMessageList.find(function (item) {
          return item.id === id;
        });
        var foundIndex = removeMessageList.indexOf(found);
...
removeLike(); //TODO: addLike

function addLike() {
  var localStorageMessages = JSON.parse(localStorage.getItem('userMessages'));
  var likeCounter = this.nextElementSibling;
  ...
  if (isNaN(id)) {
    var formalId = this.getAttribute('data-id');
    var localStorageHtml = {
      id: formalId,
      liked: true,
    ...
    var found = localStorageMessages.find(function (item) {
      return item.id === id;
    });
    found.counterLikes++;
    found.liked = true;
...
function likeListener() {
  addLikeList.forEach(function (e) {
    e.addEventListener('click', addLike);
  });
}

likeListener(); //TODO subcomment

function replyToHtml(place, userName, replyName, userSite, textArea, time, id) {
  var createListMessages = document.createElement('div');
  createListMessages.className = 'comment__reply reply';
  var htmlSubMessages = "\n\t\t\t\t\t\t\t<div class=\"reply__header\">\n\t\t\t\t\t\t\t\t<div class=\"reply__ava ava\">\n\t\t\t\t\t\t\t\t\t<div class=\"ava__img2\">\n\t\t\t\t\t\t\t\t\t\t<img class=\"ava__img1-size\" src=\"./images/face-2.png\" alt=\"\">\n\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t<a class=\"ava__web\" _blank href=\"#\">".concat(userSite, "</a>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t<div class=\"reply__group\">\n\t\t\t\t\t\t\t\t\t<h4 class=\"reply__title\">").concat(userName, "</h4>\n\t\t\t\t\t\t\t\t\t<div class=\"reply__publish-date\">").concat(timeCheck(time), "</div>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class=\"reply__content\">\n\t\t\t\t\t\t\t\t<p class=\"reply__text\">\n\t\t\t\t\t\t\t\t<span class=\"q\">").concat(replyName, ",</span>\n\t\t\t\t\t\t\t\t").concat(textArea, "</p>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class=\"reply__footer\">\n\t\t\t\t\t\t\t\t<button class=\"reply-img reply-img__second\" data-id='").concat(id, "'>Reply</button>\n\t\t\t\t\t\t\t\t<div class=\"top top__reply-padding\">\n\t\t\t\t\t\t\t\t\t<button class=\"top-p\" \">\n\t\t\t\t\t\t\t\t\t\t<img src=\"./images/like-icon.png\">\n\t\t\t\t\t\t\t\t\t</button>\n\t\t\t\t\t\t\t\t\t<button class=\"top-m\" \">\n\t\t\t\t\t\t\t\t\t\t<img src=\"./images/disLike.png\">\n\t\t\t\t\t\t\t\t\t</button>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</div>\n\t");
...
function replyFormToHtml(addNewForm) {
  var crateForm = document.createElement('div');
...
function addReply() {
  var reply = document.querySelectorAll('.reply-img');
  reply.forEach(function (e) {
    e.addEventListener('click', function () {
      var checkPrevForm = document.querySelector('.subForm');

      if (checkPrevForm) {
        var prvForm = checkPrevForm.closest('.sub-form');
        prvForm.previousElementSibling.querySelector('.reply-img').toggleAttribute('disabled');
        prvForm.remove();
      }

      e.toggleAttribute('disabled');
      var replyButton = e.closest('.reply');

      if (replyButton) {
        replyFormToHtml(replyButton);
        var subRequiredList = document.querySelectorAll('.subForm .required:not(div)');
        subRequiredList.forEach(function (e) {
        ...
        if (replyButton) {
          ...
          var _subRequiredList = document.querySelectorAll('.subForm .required:not(div)');

          _subRequiredList.forEach(function (e) {
            requiredCheck(e, 'subUserName', 'subUserEmail', 'subUserSite', 'subTextarea');
          });
...
function subCancel(trigger) {
  var cancel = document.querySelector('.subCancel');
...
function subPost(replyButton, replyName) {
  var post = document.querySelector('.subPost');
...
function addReplyPost(replyButton, replyName) {
  var userName = document.querySelector('.subUserName').value;
  var userEmail = document.querySelector('.subUserEmail').value;
  var userSite = document.querySelector('.subUserSite').value;
  var textArea = smileToImg(document.querySelector('.subTextarea').value);
  var getTime = setTimeForm();
  var subForm = document.querySelector('.sub-form');
  var id = replyButton.querySelector('.reply-img').getAttribute('data-id');
...
function localStorageFull(userName, userSite, userEmail, textArea, getTime, replyId, replyName, id) {
  ...
  if (typeof id === 'string') {
    var localStorageHTML = JSON.parse(localStorage.getItem('inHtml'));
    var localStorageHtml = {
      id: id,
      isRemoved: false,
      repliedPosts: []
    ...
    var itemHtml = localStorageHTML.find(function (item) {
      return item.id === id;
    });
    ...
    itemHtml = localStorageHTML.find(function (item) {
      return item.id === id;
    });
    itemHtml.repliedPosts.push(objectReply);
    localStorage.setItem('inHtml', JSON.stringify(localStorageHTML));
  } else {
    var localS = JSON.parse(localStorage.getItem('userMessages'));
    localS.find(function (item) {
      return item.id === id;
    }).repliedPosts.push(objectReply);
