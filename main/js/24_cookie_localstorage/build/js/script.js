  ...
  return new Promise(function (resolve, reject) {
    ...
    xhr.onprogress = function (e) {
      if (e.lengthComputable) {
        console.log("\u041F\u043E\u043B\u0443\u0447\u0435\u043D\u043E ".concat(e.loaded, " \u0438\u0437 ").concat(e.total));
      } else {
        console.log("\u041F\u043E\u043B\u0443\u0447\u0435\u043D\u043E ".concat(e.loaded));
      }
    };
...
/* harmony default export */ __webpack_exports__["default"] = (myajax);
}();
// This entry need to be wrapped in an IIFE because it need to be in strict mode.
!function() {
"use strict";
...
function renderPosts(posts, users) {
  var postsElement = document.querySelector('.posts'); //console.log(users);
  ...
  posts.forEach(function (post) {
    ...
    console.log(user);
    var htmlTemlate = "\n            <li class=\"posts__item\">\n                <h2 class=\"posts__title\">".concat(post.id, ". ").concat(post.title, "</h2>\n                <p class=\"posts__body\">").concat(post.body, "</p>\n                <div class=\"posts__info\">\n                    <p class=\"posts__user-wrapper\">\n                        Name: <span class=\"posts__user-name\">").concat(user.name, "</span>\n                        <br>Phone: <a href=\"tel:").concat(user.phone, "\">").concat(user.phone, "</a>\n                        <br>Email: <a href=\"mailto:").concat(user.email, "\">").concat(user.email, "</a>\n                        <br>www: <a href=\"").concat(user.website, "\">").concat(user.website, "</a>\n                    </p>\n                </div>\n            </li>\n        ");
...
var midNight = ~~((nextDay.getTime() - date.getTime()) / 1000); // document.cookie = 'user=Vasya; path=/;';
...
function getCookie(nameCookie) {
  ...
  var cookiesList = cookies.split('; ').map(function (item) {
    return item.split('=');
  }); //const findCookie = cookiesList.find(cookie => cookie[0] === nameCookie);

  var cookie = Object.fromEntries(cookiesList)[nameCookie];
  return cookie ? decodeURIComponent(cookie) : ''; // if (!findCookie) return '';
  // return findCookie[1];
}
...
function setCookie(nameCookie, value, options) {
  var cookie = "".concat(nameCookie, "=").concat(encodeURIComponent(value));

  if (options) {
    for (var key in options) {
      var _value = options[key];

      if (key === 'expires' && _value instanceof Date) {
        _value = _value.toUTCString();
      }

      cookie = "".concat(cookie, "; ").concat(key, "=").concat(_value);
    }
  }
...
var userList = getUserList(); // userList.shift();
// localStorage.setItem('users', JSON.stringify(userList));

var form = document.querySelector('form');
form.addEventListener('submit', function (e) {
  e.preventDefault();
  var uaerName = document.querySelector('.first-name').value;
...
function render(userList) {
  var userListWrapper = document.querySelector('.user-list');
  var f = document.createDocumentFragment();
  userList.forEach(function (user) {
    var li = document.createElement('li');
    li.classList.add('user-list__item');
    li.textContent = "User: ".concat(user.name, ", age: ").concat(user.age, ", email: ").concat(user.email);
    f.append(li);
  });
...
render(userList); //localStorage.setItem('users', JSON.stringify(userList))
//localStorage.clear();

function prepChangeCase(elemClass) {
  var tagList = document.querySelectorAll(elemClass);
  console.log(tagList);
  tagList.forEach(function (elem) {
    var reg = /\b(to|on|in|at|the|a)\b/ig;
    elem.textContent = elem.textContent.replace(reg, function (match) {
