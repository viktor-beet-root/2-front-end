...
%cDrivers: %c${this.drivers.names}`, 'font-weight:bold; color:red;font-size:14px;', 'color:white')
        ...
        showDriver(driverName) {
            ...
            if (typeof driverName === 'string' && driverName !== '') {
                ...
                //итоговая запись имени путём вырезания из строки
                nameRes = 'Driver: ' + this.drivers.names.slice(dName, dName + driverName.length);
                //запись индекса начала лицензии прав в строке licence
                lName = this.drivers.licence.indexOf(driverName);
                //итоговая запись лицензии на вождение путём вырезания из строки
                licenceRes = 'Licence: ' + this.drivers.licence.slice(lName + driverName.length + 1, lName + driverName.length + 2);
                //вывод информации
                console.log(nameRes + '\n' + licenceRes);
                //возврат для дальнешего использования
                return licenceRes;
            }
        },
        ...
        calculateTrip(distance, driverName) {
            ...
            let hoursTrip = 0;
            //константа значения максимальной безпрерывной поездки
            const maxTrip = 4;
            //константа максимальной дистанции на полном баке
            const maxTripKm = (this.tankVolume / this.fuelAverage100) * 100;
            //проверка на правильность ввода имени водителя
            if (typeof driverName === 'string' && driverName !== '') {
